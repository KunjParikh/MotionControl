...
if True or __name__ == "__main__":
  ...
  for function in params.functions:
    ...
    #error_predict = scaler_error[function.name].inverse_transform(np.hstack((error_predict, np.zeros((error_predict.shape)) )))[:,0:9]
    ...
    for i in range(10000):
      ...
      state_predict = model_state.predict(np.reshape(state, (state.shape[0], 1, state.shape[1])))
      state_predict = scaler_state[function.name].inverse_transform(np.hstack((np.zeros(state_predict.shape), state_predict )))[:,3:]
      s_e = state_predict[0]
      ...
      error_predict = model_error.predict(np.reshape(error, (error.shape[0], 1, error.shape[1])))
      error_predict = scaler_error[function.name].inverse_transform(np.hstack((np.zeros(error_predict.shape), error_predict)))[:,9:]
      #error_predict = scaler_error[function.name].inverse_transform(np.hstack((error_predict, np.zeros(error_predict.shape))))[:,0:9]
